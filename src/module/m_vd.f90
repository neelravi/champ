module vd_mod
  ...
  use force_mod, only: MFORCE_WT_PRD
  
  implicit none

  integer :: dmc_ivd
  real(dp), dimension(:, :), allocatable :: da_branch !(3, MCENT)
  real(dp), dimension(:, :), allocatable :: da_branch_sum !(3, MCENT)
  real(dp), dimension(:, :), allocatable :: da_branch_cum !(3, MCENT)
  real(dp), dimension(:, :, :), allocatable :: esnake !(3, MCENT, MWALK)
  real(dp), dimension(:, :, :), allocatable :: deriv_eold !(3, MCENT, MWALK)
  real(dp), dimension(:, :, :, :), allocatable :: ehist !(3, MCENT, MWALK, 0:MFORCE_WT_PRD)
