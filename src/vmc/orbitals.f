        ...
        if(i3dsplorb.eq.2) then
          do i=1,nelec
            ...
            do iorb=1,norb+nadorb
              ...
              call spline_mo (x(1,i),iorb,orb(iorb,i),dorb(iorb,i,:),ddorb(iorb,i),ier)
            enddo
            ...
            if(ier.eq.1) then
              ...
              do iorb=1,norb+nadorb
                ...
                do m0=1,n0_nbasis(i)
                  m=n0_ibasis(m0,i)
                  orb(iorb,i)=orb(iorb,i)+coef(m,iorb,iwf)*phin(m,i)
...
!     keep the old localization code if no vectorization instructions available
      ...
      if(ipr.ge.0) then
        do iorb=1,norb+nadorb
          write(ounit,'(''iorb,orb='',i4,1000f15.11)') iorb,(orb(iorb,i),i=1,nelec)
        enddo
         do iorb=1,norb+nadorb
...
c     it maybe replaced and simplyfied bbu transposing coeff matrix and orb
      ...
      call dgemm('n','n', nadorb, nelec, nbasis,1.d0,  coef_t,  nadorb, phin,nbasis,0.d0,  orb(norb+1:norb+nadorb,:),  nadorb)
...
c     ddorb(iorb,i)=0.d0
c     do 25 m=1,nbasis
c     orb(iorb,i)=orb(iorb,i)+coef(m,iorb,iwf)*phin(m,i)
