...
!
! DESCRIPTION:
!> Optimize the wave function parameters using the SR method
!
...
contains
    ...
    subroutine optwf_sr
        ...
        integer :: i, iflag, iter, miter, im, keep_track, stage_track, max_conv, config_conv, max_conf, Na, Nave, Nmin, j, k, ic, n_stage, max_stage
        ...
        do while (n_stage.le.max_stage)
           ...
           if (ifssd.gt.0) then
              do while (stage_track.le.Nmin)
                 ...
                 call macro_iteration(energy_sav, alpha_omega, omega, sigma_sav, im, adiag, dparm_norm, iflag, cent_history, keep_track, stage_track, error_bar_fssd, config_conv)
              enddo
           endif
           ...
           do while (.not. check_config_conv)
              ...
              !Convergence for configuration !! CHANGE TO CONV ON FORCES ??
        ...
        if (ifssd .gt. 0) write (ounit, '(/,''Calculation terminated with alfgeo, error_bar_fssd, keep_track, config_conv, im '', d10.3, '' '', d10.3, i5, i5, i5)') alfgeo, error_bar_fssd, keep_track, config_conv, im
      ...
      subroutine macro_iteration(energy_sav, alpha_omega, omega, sigma_sav, im, adiag, dparm_norm, iflag, cent_history, keep_track, stage_track, error_bar_fssd, config_conv)
        use optwf_func, only: ifunc_omega, omega0, n_omegaf, n_omegat, omega_hes
        use control_vmc, only: vmc_nblk
        use optwf_corsam, only: energy, energy_err
        use force_analy, only: alfgeo
        ...
        integer :: iter, im, iflag, keep_track, stage_track, config_conv
      ...
      do iter = 1, nopt_iter
         ...
         call micro_iteration(adiag, dparm_norm, iflag, iter, cent_history, im, keep_track, stage_track, config_conv)             
    ...
    subroutine micro_iteration(adiag, dparm_norm, iflag, iter, cent_history, im, keep_track, stage_track, config_conv)

      use optwf_contrl, only: nparm
      use optwf_corsam, only: energy
      use atom, only: cent, ncent

      integer :: i, miter, n_sr, iflag, iter, k, ic, keep_track, stage_track, j, config_conv, im
...
6        continue
         ...
         if (iforce_analy .gt. 0) then
            ...
            write  (ounit, '(''keep_track is '',i5, '' for config_conv '', i5,'', im '',i5,'', iter '',i5,'',  miter '',i5)') keep_track, config_conv, im, iter, miter
               do ic = 1,ncent
                  do k = 1,3
    ...
    subroutine config_check(keep_track, max_conv, cent_history, error_bar_fssd, cent_ave, check_config_conv, Nave)
      ...
      real(dp), dimension(:, :, :) :: cent_history
      real(dp) :: ave_dist_a, ave_dist_b, stderr_dist_a, stderr_dist_b, error_bar_fssd, R_th, R_max, R_upper
      ...
      if (track > 2 * N_a + 2) then 
         do j = N_a,track-N_a
            ...
            if (R_conv(j) .ge. R_th .and. R_conv(j) .lt. R_upper) then
